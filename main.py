...
def main() -> None:
    ...
    for depth in range(1, 6):
        ...
        if os.path.exists(path):
            ...
            print(df["score"].mean())
            df.to_csv(path)
    path = f"data/monte-carlo_vs_monte-carlo_rollout_{rollouts}_monte_carlo_depth_{monte_carlo_depth}_simulations_{simulation_count}.csv"
    if os.path.exists(path):
        print(f"Found data for {path}")
    else:
        print(f"Running sims for {path}")
        game_stats = run_monte_carlo_vs_monte_carlo(simulation_count, rollout=rollouts, depth=monte_carlo_depth)
        df = pd.DataFrame([asdict(stats) for stats in game_stats])
        print(df["score"].mean())
        df.to_csv(path)
    path = f"data/monte-carlo_vs_expectimax_rollout_{rollouts}_expectimax_depth_{depth}_monte_carlo_depth_{monte_carlo_depth}_simulations_{simulation_count}.csv"
    if os.path.exists(path):
        print(f"Found data for {path}")
    else:
        print(f"Running sims for {path}")
        game_stats = run_monte_carlo_vs_expectimax(simulation_count, rollout=rollouts, depth=depth, monte_carlo_depth=monte_carlo_depth)
        df = pd.DataFrame([asdict(stats) for stats in game_stats])
        print(df["score"].mean())
        df.to_csv(path)
    path = f"data/monte-carlo_vs_random_rollout_{rollouts}_monte_carlo_depth_{monte_carlo_depth}_simulations_{simulation_count}.csv"
    if os.path.exists(path):
        print(f"Found data for {path}")
    else:
        print(f"Running sims for {path}")
        game_stats = run_monte_carlo_vs_random(simulation_count, rollout=rollouts, monte_carlo_depth=monte_carlo_depth)
        df = pd.DataFrame([asdict(stats) for stats in game_stats])
        print(df["score"].mean())
        df.to_csv(path)
    path = f"data/random_vs_monte-carlo_rollout_{rollouts}_monte_carlo_depth_{monte_carlo_depth}_simulations_{simulation_count}.csv"
    if os.path.exists(path):
        print(f"Found data for {path}")
    else:
        print(f"Running sims for {path}")
        game_stats = run_random_vs_monte_carlo(simulation_count, rollout=rollouts, depth=monte_carlo_depth)
        df = pd.DataFrame([asdict(stats) for stats in game_stats])
        print(df["score"].mean())
        df.to_csv(path)
